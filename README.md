...
### `text_results` folder     
Contains files in `.txt` format from the results of `step3_fit_and_eval.py`. These data, together with the information in ` metadata_features.csv `, are used in `step4_analyze_results.py` for analyzing results and in `results_visualization.ipynb` for visualizing them.
...
If an error occurs during feature extraction, a `.txt` file with the sample ID is stored here. Each file contains the error message for that sample. If no error occurs during feature extraction, this folder will remain empty.
...
- `sarcomere_length_std`: Standard deviation of sarcomere length [µm]    
- `sarcomere_area_ratio`: Ratio of cell mask area occupied by sarcomeres [µm]   
- `z_length_mean`: Mean Z-band length [µm]   
- `n_zbands`: Number of Z-bands   
- `cell_mask_area`: Cell area [µm²]
**Note**: only three of these features `sarcomere_area_ratio`, `z_length_mean` and `n_zbands` are later used for the clustering process.
...
`PC_train.txt` and `PC_test.txt` are used in visualizing the clusters.
